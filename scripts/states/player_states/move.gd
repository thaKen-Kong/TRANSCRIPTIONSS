...
# STATE LIFECYCLE
func enter(_entity):
	_update_energy_ui(true) # instant update
...
func _drain_atp(delta):
	...
	# Prevent drain if disabled (Hub)
	if not stats.atp_drain_enabled:
		return
...
func _update_energy_ui(instant := false):
	if not player or not player.player_ui:
		return
	...
	if not bar or not label:
		push_warning("ATP UI nodes not found!")
		return
	...
	if instant:
		bar.value = PlayerInfo.player_info.atp_points
	else:
		ui_tween = get_tree().create_tween()
		ui_tween.tween_property(bar, "value", PlayerInfo.player_info.atp_points, TWEEN_DURATION).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)

	label.text = str(PlayerInfo.player_info.atp_units)
